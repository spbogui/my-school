...
describe('Room e2e test', () => {
    ...
    it('should load Rooms', async () => {
        ...
        expect(await roomComponentsPage.getTitle()).to.eq('mySchoolApp.room.home.title');
    });

    it('should load create Room page', async () => {
        await roomComponentsPage.clickOnCreateButton();
        roomUpdatePage = new RoomUpdatePage();
        expect(await roomUpdatePage.getPageTitle()).to.eq('mySchoolApp.room.home.createOrEditLabel');
    ...
    it('should create and save Rooms', async () => {
        ...
        await roomComponentsPage.clickOnCreateButton();
        await promise.all([roomUpdatePage.setLabelInput('label'), roomUpdatePage.roomTypeSelectLastOption()]);
        expect(await roomUpdatePage.getLabelInput()).to.eq('label');
    ...
    it('should delete last Room', async () => {
        ...
        expect(await roomDeleteDialog.getDialogTitle()).to.eq('mySchoolApp.room.delete.question');
